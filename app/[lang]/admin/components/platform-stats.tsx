...
export function PlatformStats() {
  ...
  const initialTab = ((searchParams?.get("statsTab") as TabType) || "users")
  const [activeTab, setActiveTab] = useState<TabType>(initialTab)

  useEffect(() => {
    // Keep local state in sync when the URL changes externally
    const tab = (searchParams?.get("statsTab") as TabType) || "users"
    setActiveTab(tab)
  }, [searchParams])
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        ...
        <Button
          ...
          variant={activeTab === "showroom" ? "default" : "outline"}
          className={activeTab === "showroom" ? "bg-[#B8071C] hover:bg-[#910515]" : ""}
        >
          <Store className="w-4 h-4 mr-2" />
          Showroom ({stats?.showroom.total || 0})
        </Button>
      </div>
