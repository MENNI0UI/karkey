...
export default function AuctionBidClient({ id, initialAuction }: AuctionBidClientProps) {
    const { t, language } = useTranslation()
    const [auction, setAuction] = useState<any>(initialAuction)
    const [loadingAuction, setLoadingAuction] = useState(!initialAuction)
    ...
    const photos = (auction?.photos ?? auction?.vehicle?.photos ?? (auction?.image ? [auction.image] : [])).map((url: string, i: number) => ({ id: i, url: normalizePhotoUrl(url) }))
    const _priceCandidates = [auction?.current_bid, auction?.starting_price, auction?.price, auction?.vehicle?.price, auction?.startingPrice]
    ...
    const sellerData: SellerData | undefined = auction?.seller ? {
        ...
        avatar: normalizeAvatarUrl(auction.seller.profile_picture ?? auction.seller.avatar) ?? "/placeholder.svg"
    } : undefined
