...
describe('Rate Limiter', () => {
    ...
    it('allows requests within the limit', async () => {
        ...
        for (let i = 0; i < limit; i++) {
            expect(await isRateLimited(key, { limit, windowMs })).toBe(false);
        }
    });
    ...
    it('blocks requests exceeding the limit', async () => {
        ...
        expect(await isRateLimited(key, { limit, windowMs })).toBe(true);
    });
    ...
    it('resets after window expires', async () => {
        ...
        // Block logic
        expect(await isRateLimited(key, { limit, windowMs })).toBe(false); // Count = 1
        expect(await isRateLimited(key, { limit, windowMs })).toBe(true);  // Count = 2 (blocked)
