...
export interface AuctionFiltersSidebarRef {
  applyFilters: () => void;
}

export const AuctionFiltersSidebar = forwardRef<
  AuctionFiltersSidebarRef,
  {
...
>(({
  ...
  // Use unified saved search hook - complete isolation from direct-sales
  ...
  const pushParams = useCallback((newParams: URLSearchParams) => {
    ...
    router.push(`/${language}/auctions?${qs}`, { scroll: false })
  }, [language, router])
  ...
  // Sync from URL changes (external nav)
  useEffect(() => {
    setLocalFilters({
      make: getParams("make"),
      model: getParams("model"),
  ...
  useEffect(() => {
    // On mobile, we wait for manual "Show Results" click
    if (mobile) return
  ...
  const filteredModels = useMemo(() => {
    ...
    selectedMakes.forEach(m => {
      const k = Object.keys(modelsByMake).find(key => normalize(key) === m)
      if (k && modelsByMake[k]) {
        modelsByMake[k].forEach(mod => combined.add(mod))
        found = true
      } else if (modelsByMake[m]) {
        modelsByMake[m].forEach(mod => combined.add(mod))
        found = true
      }
    })
    if (found) return Array.from(combined).sort()
    return options?.models ?? DEFAULTS.models
  }, [selectedMakes, modelsByMake, options?.models])

  // --- Results Count Logic (Debounced) ---
  ...
  useEffect(() => {
    ...
    const fetchCount = async () => {
      ...
      const url = qs ? `/api/auctions/approved?${qs}` : `/api/auctions/approved`
      try {
    ...
    const t = setTimeout(fetchCount, 800)
  ...
  return (
    <aside className={asideClass} style={{ zIndex: 120 }}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-2 scrollbar-thin">
        ...
        {/* Year */}
        <FilterSection title={t("filters.year_range")}>
          <CustomMultiSelect
        ...
        {/* Transmission */}
        <FilterSection title={t("filters.transmission_label")}>
          <FilterChips
        ...
        <FilterSection title={t("filters.price_range")}>
          <FilterRange
            ...
            minPlaceholder="0" maxPlaceholder="250000"
            step="10000"
          />
        </FilterSection>
        ...
        <FilterSection title={t("filters.mileage_range") || "Mileage"}>
          <FilterRange
            ...
            labelMin={t("filters.min")} labelMax={t("filters.max")}
            step="10000"
          />
        </FilterSection>
        ...
        {/* Doors */}
        <FilterSection title={t("filters.doors")}>
          <FilterChips
        ...
        {/* Colors */}
        <FilterSection title={t("filters.exterior_color")}>
          <FilterColors
        ...
        <FilterSection title={t("filters.interior_color")}>
          <FilterColors
        ...
        {/* Original Paint */}
        <FilterSection title={t("filters.original_paint")}>
        ...
        <div className="flex flex-col gap-3">
          ...
          >
            ...
            <span className="font-serif tracking-wide">
              {resultCount === null ? "..." : `${resultCount} ${t("nav.auctions")}`}
            </span>
          </button>
...
AuctionFiltersSidebar.displayName = "AuctionFiltersSidebar"
